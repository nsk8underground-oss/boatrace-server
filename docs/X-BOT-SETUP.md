...
AI予想を X に自動投稿するBOTの設定方法です。**追加費用はかかりません**（X APIの無料枠 + cron-job.org の無料枠）。

定期実行は cron-job.org から行います（手順は [EXTERNAL-CRON.md](EXTERNAL-CRON.md)）。GitHub Actions の定期実行はこのリポジトリでは起動が不安定で使えなかったため、手動実行専用にしてあります。
...
> **パスワードについて**
> `SITE_PASSWORD` が4桁の数字だと、総当たりで1万通りしかなく現実的に破られます。
> サーバー側でも「同じIPから8回まちがえたら10分締め出す」対策を入れていますが、
> Vercelの環境変数 `SITE_PASSWORD` は**英数字を混ぜた12文字以上**にしてください。
...
- 定期実行は cron-job.org 側の設定です。投稿が止まったときは、まず cron-job.org の実行履歴を確認してください
